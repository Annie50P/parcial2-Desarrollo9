...
2. **Mejoras (Postventa):** MÃ³dulo de GarantÃ­a SÃ³lido (reportes, subida a MinIO de evidencias), Panel Administrativo extendido para tÃ©cnicos, Notificaciones y transacciones refinadas.
3. **Escalabilidad:** CachÃ© (Redis), CDN para MinIO, Pipelines de CI/CD avanzados, Observabilidad.
...
- `/src/services` (stripe, clerk, minio)
...
- **`POST /api/uploads` (Protegido: User/Admin)**
  ...
  - *Respuesta (200 OK):* `{ url: "https://minio.tominio.com/bucket/file.jpg" }`
...
- **`[Issue 68]` Configurar Repositorios y DB (Backend/DB)**
...
- **`[Issue 69]` Configurar Vite + UI Base e Integrar Clerk (Frontend/Auth)**
  - *DescripciÃ³n:* Montar proyecto en React/Vite e implementar login/registro. **IMPORTANTE:** El diseÃ±o debe guiarse por la skill "frontend-design", optando por un estilo estÃ©tico profesional.
...
- **`[Issue 70]` Crear CRUD de Productos (Backend)**
  - *Dependencia:* Depende de #68
...
- **`[Issue 71]` Listado y Detalle de Productos en UI (Frontend)**
  - *Dependencia:* Depende de #69 y #70
  - *DescripciÃ³n:* Implementar diseÃ±o de tienda exhibiendo datos del API. **IMPORTANTE:** Aplicar diseÃ±o original premium (skill "frontend-design").
...
- **`[Issue 71]` Implementar Carrito en local (Frontend)** *(Nota: Mismo sub-ID que catÃ¡logo en GitHub para Zustand)*
  - *DescripciÃ³n:* GestiÃ³n de estado global para carrito (Zustand).
...
- **`[Issue 73]` Webhooks de Stripe y CreaciÃ³n de Orden (Backend/DB)**
  - *Dependencia:* Depende de #72
...
- **`[Issue 78]` Vista de Historial de Ã“rdenes (Frontend)**
  - *Dependencia:* Depende de #73
  - *DescripciÃ³n:* Pantalla "Mis Pedidos" para usuarios. **DISEÃ‘O:** Utilizar estÃ©tica y visual superior guiada por la skill de frontend (profesional/elegante).
  - *Criterios de AceptaciÃ³n:* Lista de Ã³rdenes pertenecientes al user logged in.
  - *Prioridad:* Media | *Etiquetas:* frontend

### Sprint 3: GarantÃ­a y Admin MVP
- **`[Issue 74]` Integrar MinIO para ImÃ¡genes (Backend)**
  - *DescripciÃ³n:* Configurar SDK compatible con S3 para subir imÃ¡genes (productos, evidencias) a un bucket de MinIO.
  - *Criterios de AceptaciÃ³n:* FunciÃ³n que sube foto y devuelve URL (almacenamiento local/nube de MinIO).
  - *Prioridad:* Alta | *Etiquetas:* backend, storage
- **`[Issue 75]` Formulario de Reporte de GarantÃ­a UI (Frontend/GarantÃ­a)**
  - *DescripciÃ³n:* Interface para que usuarios hagan efectivos reclamos dentro de 90 dÃ­as. **IMPORTANTE:** Aplicar diseÃ±o original superior basado en skill "frontend-design" (uso de tema profesional, motion y tipografÃ­a premium).
  - *Dependencia:* Depende del Endpoint Ticket GarantÃ­a
  - *Criterios de AceptaciÃ³n:* Form sube imagen (a MinIO) y texto y crea ticket exitoso.
  - *Prioridad:* Media | *Etiquetas:* frontend
- **`[Issue 76]` Endpoints para Tickets de GarantÃ­a (Backend)**
  - *DescripciÃ³n:* API para manejo de reportes.
  - *Dependencia:* Depende del upload a MinIO
  - *Criterios de AceptaciÃ³n:* Endpoint POST para usuario, GET/PUT para admins.
  - *Prioridad:* Alta | *Etiquetas:* backend, db
- **`[Issue 77]` Panel Admin Dashboard (Frontend/Admin)**
  - *DescripciÃ³n:* Vista protegida solo para rol admin. **IMPORTANTE:** Aplicar diseÃ±o original profesional basado en skill "frontend-design".
  - *Criterios de AceptaciÃ³n:* Mostrar lista de Ã³rdenes y reportes de garantÃ­a activos para gestionar.
  - *Prioridad:* Media | *Etiquetas:* frontend, auth, auth

## Trabajo en Equipo y ParalelizaciÃ³n
- **Persona A (Frontend):** ConfiguraciÃ³n de Vite, Clerk Auth, UI de CatÃ¡logo, Carrito. Implementar DISEÃ‘O PROFESIONAL para todas las vistas usando la "frontend-design skill" aportada.
- **Persona B (Backend):** Esquemas DB en MongoDB, Bun Server, Rutas de Productos, Clerk Webhooks (sync DB).
*Dependencias clave:* Frontend puede mockear datos mientras Backend desarrolla. IntegraciÃ³n de MinIO (Backend) bloquea la Subida de Evidencia (Frontend).
